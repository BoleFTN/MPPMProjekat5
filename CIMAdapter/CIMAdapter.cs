    ...
    public class CIMAdapter
    {
        ...
        private bool DoTransformAndLoad(Assembly assembly, ConcreteModel concreteModel, SupportedProfiles extractType, out Delta nmsDelta, out string log)
        {
            ...
            try
            {
                ...
                switch (extractType)
                {
                    case SupportedProfiles.Projekat5:
                        {
                            TransformAndLoadReport report = Projekat5Importer.Instance.CreateNMSDelta(concreteModel);

                            if (report.Success)
                            {
                                nmsDelta = Projekat5Importer.Instance.NMSDelta;
                                success = true;
                            }
                            else
                            {
                            ...
                            Projekat5Importer.Instance.Reset();
